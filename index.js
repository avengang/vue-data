...
var util = {
  ...
  $setSingle: function(k, value, dist) {
    ...
    if(indexOrKey) {
      if(Object.prototype.toString.call(dist[k]) === '[object Array]') {
        ...
        dist.$set(dist[k], indexOrKey, this.$deepCopy(value))
      ...
      if(Object.prototype.toString.call(dist[k]) === '[object Array]') {
        ...
        for(var j=0;j<value.length;j++) {
          dist.$set(dist[k], j, this.$deepCopy(value[j]))
        }
      } else if(Object.prototype.toString.call(dist[k]) === '[object Object]') {
        dist[k] = {}
        for(var _key in value) {
          dist.$set(dist[k], _key, this.$deepCopy(value[_key]))
        }
      } else {
        dist[k] = this.$deepCopy(value)
      }
    }
  },
  $set: function(src, dist) {
    for(var k in src) {
      this.$setSingle(k, src[k], dist)
    }
  },
  ...
  $getArgMethodParam: function(arg) {
    var result = []
  ...
  $getViewtag: function(vm, name_tags) {
    if(vm._uid === 1) {
    	return 'app'
    }
    var oldVm = vm
    if(vm._props.viewtag) {
      if(!name_tags[oldVm.configviewname][vm._props.viewtag]) {
      	...
      	return vm._props.viewtag + '_' + this.$getUuid()
      }
    }
    vm = vm.$parent
    while(vm) {
      if(vm.$$viewtag) {
        if(!name_tags[oldVm.configviewname][vm.$$viewtag + '_' + oldVm.configviewname]) {
          ...
          return vm.$$viewtag + '_' + oldVm.configviewname + '_' + this.$getUuid()
        }
      }
      vm = vm.$parent
    }
    if(!name_tags[oldVm.configviewname][oldVm.configviewname]) {
      ...
      return oldVm.configviewname + '_' + this.$getUuid()
    }
  }
}
var name_tags = {}
